...
window.onload = function(){
  ...
  function showAddWin(oData, nIndex){
    ...
    oHistory = getHistoryList();
		$("body").append("<div class='mod_win_wrapper' "+nIndex+"><div class='mod_win' id='editor_win'>"+oTop+"<div class='modWinCont' style='max-height: 90%; overflow-y: auto'>"+sForm+oHistory+"</div><div>"+sButton+"</div></div></div>");
    $("body").css("overflow-y", "hidden");
    ...
    if($(".color_list .selected").length < 1) {
      $(".color_list .color_list_item").eq(0).addClass("selected");
		...
		// count icos height
		//var nGlobal height = $("#editor_win").height();
